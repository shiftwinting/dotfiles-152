...
map('n', '<Space>', "v:lua.require'indentInsert'.indent('a')", expr )
map( 'v', '<Space>', 'I', nore )
...
map( 'n', 'gx', "<cmd>!xdg-open <cWORD> &<CR><CR>", cmd )
...
--[[ LSP bindings ]]--
-- jump diagnostics
map( 'n', '<up>',   '<cmd>lua vim.diagnostic.goto_prev{ float = false }<cr>', cmd )
...
map( 'n', '<leader>d',
'<cmd>lua vim.lsp.diagnostic.show_line_diagnostics({ focusable=false, max_height=30, max_width=100, show_header=false })<cr>',
na )
